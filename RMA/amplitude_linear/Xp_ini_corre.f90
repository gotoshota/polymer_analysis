Program ini_corre
    implicit none

    character filename*50
    double precision dummy, dt
    double precision,allocatable :: pos(:,:,:), Xp(:,:), Xp2(:), Xp2_temp(:)
    double precision,parameter :: pi = acos(-1.0d0)
    integer n, atoms, i, mol, j, k, l, dump, step, frame, delta

    print*,'Input trajectory filename.'
    read*,filename
    
    print*,'Input number of steps (Integer)'
    read*,step

    print*,'Input dump frequency (integer)'
    read*,dump

    print*,'Input timestep (dt : float)'
    read*,dt

    print*,'Input the number of particles in this system (integer)'
    read*,atoms

    print*,'Input the number of atoms in a molecule (integer)'
    read*,n

    !nmol = nparticle / natom
    frame = step / dump

    print*,'#################################################'

    open(17,file=filename,status='old')

    mol = atoms / N
    allocate(pos(3,N,mol))
    allocate(Xp(3,0:N-1)) !(xyz , modes)
    allocate(Xp2(0:N-1))
    allocate(Xp2_temp(0:N-1))
    Xp2=0.0d0
    Xp2_temp=0.0d0
    
   !Input coordinates
    do l = 0,frame
        do j=1,9
            read(17,'()')
        enddo
        do j=1,mol
            do k = 1 , n
                read(17,*)dummy,pos(:,k,j)
            enddo
        enddo
        do i = 1,mol !each molecule
            do j = 0,N-1 !Rouse mode
                Xp = 0.0d0
                do k = 1,N  !atoms in a molecule
                    Xp(:,j) = Xp(:,j) + pos(:,k,i)* cos((dble(k)-0.50d0)* DBLE(j)* pi/ dble(N))
                enddo
                if (j == 0) then
                    delta = 1
                else
                    delta = 0
                endif
                Xp(:,j) = Xp(:,j)*sqrt(dble(2-delta)/dble(N))
                Xp2_temp(j) = XP2_temp(j) + dot_product(Xp(:,j),Xp(:,j))
            enddo
        enddo
        do j = 0,N-1
            Xp2_temp(j) = Xp2_temp(j)/mol
            Xp2(j) = Xp2(j) + Xp2_temp(j)
            Xp2_temp(j)=0.0d0
        enddo
    enddo
    Xp2(:)=Xp2(:)/DBLE(frame+1)

    close(17)
    open(17,file='Xp_inicorre.xvg',status='replace')
    write(17,*)'#mode p, Rouse Amplitude Xp**2, sin(pi*p/2N)**2'
    do i = 0,N-1
        write(17,'(I4,1X,F30.15,1X,F30.15)')i,Xp2(i),sin(DBLE(i)*pi/dble(N)/2.0)**2.0
    enddo
    close(17)

end Program ini_corre
