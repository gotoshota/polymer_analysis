        ...
        do j = 0,nmol-1 !polychain
            do p = 0,natom-1 !mode
                do k = 1,natom !beads in a chain
                    Xp(:,j*natom+p+1,i)= Xp(:,j*natom+p+1,i) + &
                    pos(:,j*natom+k,i)*cos(DBLE(p)*pi*(DBLE(k)-0.50d0)/dble(natom))
                enddo
                if (p == 0) then
                    Xp(:,j*natom+p+1,i) = Xp(:,j*natom+p+1,i) / SQRT(DBLE(natom))
                else
                    Xp(:,j*natom+p+1,i) = Xp(:,j*natom+p+1,i) * SQRT(2.0d0) / SQRT(DBLE(natom))
                endif
            enddo
        enddo
    enddo
