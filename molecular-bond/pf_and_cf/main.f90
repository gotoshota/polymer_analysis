! This program calculates how long it takes that two polymers which overlapped each other become "unoverlapped state".
! Note: Be careful with variable name. It is confusing that overlap and molecular-bond represent the same state (r_ij <= r_o).
program main
    !$ use omp_lib
    use prm_list
    use read_prm
    ...
    do ii = 1, 20
        ...
        cf_bond = 0.0d0
        !$ omp do private(j,k,l,m,i_tmp_cf) reduction(+: int_summation, cf_bond)
        do i = 1, npoint
            i_tmp_cf    = 0
            do j = 0, nframe - TargetFrame(i)
                do k = 1, nmol 
                    do l = 1, nmol
                        int_summation = 0
                        do m = j, j+TargetFrame(i) 
                            int_summation = int_summation + flag_bond(l,k,m)
                        enddo
                        if (int_summation .eq. TargetFrame(i)+1) then
