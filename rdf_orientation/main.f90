...
! #######################################################################
        ...
        do j = 1, nmol !!set j th particle on origin
            do k = 1, nmol
                if (j .ne. k) then
                    ...
                    size_dot_pro = SQRT(inertia_tensor(1,3,j,i)*inertia_tensor(1,3,j,i)&
                                 +  inertia_tensor(2,3,j,i)*inertia_tensor(2,3,j,i)&
                                 +  inertia_tensor(3,3,j,i)*inertia_tensor(3,3,j,i))

                    size_dot_pro = size_dot_pro * &
                                 SQRT(inertia_tensor(1,3,k,i)*inertia_tensor(1,3,k,i)&
                                 +  inertia_tensor(2,3,k,i)*inertia_tensor(2,3,k,i)&
    ...
    rdf_cm_cm            = dble(rdf_cm_cm) / dble(nmol * num_calc_frame)
    rdf_cm_cm_theta      = dble(rdf_cm_cm_theta) / dble(nmol * num_calc_frame)
    ...
    do i = 1, r_max
        ...
        rdf_cm_cm_theta(i)              = rdf_cm_cm_theta(i) / (dv * rho_cm_cm)
    enddo
    call system_clock(tend)
    t = real(tend - tbegin) / CountPerSec
    
    open (16, file ='rdf_orientation.xvg',status='replace')
